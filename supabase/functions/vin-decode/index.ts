...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!/^[A-HJ-NPR-Z0-9]{17}$/.test(vin)) {
      return new Response(JSON.stringify({ error: "VIN must be exactly 17 valid characters." }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    return new Response(JSON.stringify({ vehicle: {
      vin,
      year: Number(pick("ModelYear")) || null,
      make: pick("Make"),
      model: pick("Model"),
      trim: pick("Trim"),
    ...
    return new Response(JSON.stringify({ error: error instanceof Error ? error.message : "VIN decode failed." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
});
